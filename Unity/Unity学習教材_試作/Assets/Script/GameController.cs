...
public class GameController : MonoBehaviour {
    ...
    void Start()
    {
        ...
        GameClear = PlayerPrefs.GetInt("GameClearKey", 0); //セーブデータの呼び出し。前回クリアしていれば1になる
        if (GameClear == 1)
        {
    ...
    public void Execution() //入力結果を実行する
    {
        ...
        if (TrueFalseJudgment()) //正解かどうか判定
        {
            ...
            player.transform.position = startPlayerPos;//playerの位置を初期化
            player.transform.Translate(int.Parse(inputFieldA.text), int.Parse(inputFieldB.text), int.Parse(inputFieldC.text));//入力された数字のとおりにplayerが移動
            
             
            messageText.text = "正解";
            gameClear = 1;
            PlayerPrefs.SetInt("GameClearKey",1);//クリアしたらセーブ
            GameObject.Find("LeftCanvas").transform.Find("Next").gameObject.SetActive(true);//クリアしたタイミングでNextボタンをアクティブにする．
        }
        else
        {
            ...
            if (inputPos.x == correctPos.x)
            {
                ...
                else
                {
                    ...
                    {
                        messageText.text = "<color=red>y</color>が間違っています";
                    }
                    else
                    {
                        messageText.text = "<color=red>y</color>と<color=red>z</color>が間違っています";
                    }
                }
            }
            else
            {
                ...
                else
                {
                    if (inputPos.z == correctPos.z)
                    {
                        messageText.text = "<color=red>x</color>と<color=red>y</color>が間違っています";
                    }
                    else
                    {
                        messageText.text = "<color=red>x</color>と<color=red>y</color>と<color=red>z</color>が間違っています";
                    }
                }
            }
    ...
    public static int GameClear //gameClear変数のプロパティ
    {
